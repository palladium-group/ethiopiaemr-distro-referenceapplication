...
if [ -z "$SPA_CONFIG_URLS" ]; then
  sed -i -e 's/"$SPA_CONFIG_URLS"//' "/usr/share/nginx/html/openmrs/spa/index.html"
...
# we support two formats, a comma-separated list or a space separated list
  ...
  sed -i -e 's/"$SPA_CONFIG_URLS"/$SPA_CONFIG_URLS/' "/usr/share/nginx/html/openmrs/spa/index.html"
fi
